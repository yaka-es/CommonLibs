...
{
	Timeval nowTime;
	if (nowTime.mTimeval.tv_sec < mTimeval.tv_sec)
		return false;
	if (nowTime.mTimeval.tv_sec > mTimeval.tv_sec)
		return true;
	if (nowTime.mTimeval.tv_usec > mTimeval.tv_usec)
		return true;
	return false;
}

double Timeval::seconds() const { return ((double)mTimeval.tv_sec) + 1e-6 * ((double)mTimeval.tv_usec); }
...
string Timeval::isoTime(time_t t, bool isLocal)
{
	...
	if (isLocal)
		...
		gmtime_r(&t, &tBuf);
	snprintf(buf, sizeof(buf) - 1, "%04d-%02d-%02dT%02d:%02d:%02d%s", tBuf.tm_year + 1900, tBuf.tm_mon + 1,
		tBuf.tm_mday, tBuf.tm_hour, tBuf.tm_min, tBuf.tm_sec, isLocal == false ? "Z" : "");
	return string(buf);
}

void Timeval::isoTime(time_t t, std::string &result, bool isLocal) { result = isoTime(t, isLocal); }
