...
int RefCntBase::decRefCnt() const
{
	...
	// The typeid(this).name() doesnt add anything because it is just the name of the class here, not the derived
	// class.
	LOG(DEBUG) << " " << (void *)this << " " << LOGVAR2("refcnt after", saveRefCnt);
	// printf("decRefCnt %p mRefCnt=%d\n",this,mRefCnt);
	// The refcnt is created with a value of zero, so when the last one is deleted it will come in with a value of
	// zero.
	if (saveRefCnt <= 0) {
...
ostream &operator<<(std::ostream &os, std::ostringstream &ss) { return os << ss.str(); }
...
string format(const char *fmt, ...)
{
	...
	if (n <= 199) {
		...
		if (n > 5000) {
			LOG(ERR) << "oversized string in format";
			n = 5000;
		}
...
const string timestr(unsigned fieldwidth, bool addDate) // Use to pick the number of chars in the output.
{
	...
	if (addDate)
		...
		result = format(" %04d-%02d-%02dT%02d:%02d:%02d.%1d", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
...
{
	if (howlong <= 0.00001)
		return; // Less than 10 usecs, forget it.
...
int cstrSplit(char *in, char **pargv, int maxargc, const char *splitchars)
{
	...
	int argc = 0;
	while (argc < maxargc) {
		while (*in && strchr(splitchars, *in)) {
			in++;
		} // scan past any splitchars
		if (!*in)
			return argc;	  // return if finished.
...
void printPrettyTable(prettyTable_t &tab, ostream &os, bool tabSeparated)
{
	...
	for (unsigned nrow = 0; nrow < tab.size(); nrow++) {
		...
		for (unsigned col = 0; col < maxcols && col < row.size(); col++) {
			...
			if (0 == strcmp(val, "_")) {
				val = "";
			} // Dont print "_", used to easily format the header fields.
			if (tabSeparated) {
				if (col)
					os << "\t";
				os << val;
...
ostream &operator<<(std::ostream &os, const Statistic<int> &stat)
{
	stat.text(os);
	return os;
}
ostream &operator<<(std::ostream &os, const Statistic<unsigned> &stat)
{
	stat.text(os);
	return os;
}
ostream &operator<<(std::ostream &os, const Statistic<float> &stat)
{
	stat.text(os);
	return os;
}
ostream &operator<<(std::ostream &os, const Statistic<double> &stat)
{
	stat.text(os);
	return os;
}
...
string replaceAll(const std::string input, const std::string search, const std::string replace)
{
	...
	while (index1 < output.size()) {
		try {
			...
			LOG(ERR) << "string replaceAll error" << LOGVAR(index1) << LOGVAR(input) << LOGVAR(search)
				 << LOGVAR(replace) << LOGVAR(output);
			break;
		}
	}
...
string firstlines(string msgstr, int n)
{ // n must be >=1
...
static struct BaseNDecoder {
	BaseNDecoder()
	{
		...
		for (unsigned i = 0; i < 64; i++) {
			sMapBase64Reverse[sMapBase64[i]] = i;
		}
		...
		for (unsigned ch = '0'; ch <= '9'; ch++)
			sMapHexReverse[ch] = ch - '0';
		for (unsigned ch = 'A'; ch <= 'Z'; ch++)
			sMapHexReverse[ch] = ch - 'A' + 10;
		for (unsigned ch = 'a'; ch <= 'z'; ch++)
			sMapHexReverse[ch] = ch - 'a' + 10;
	}
} sBaseNDecoderInit;
...
// (pat) Decode base64 encoded buffer of characters to a string of binary data.  Ignore interspersed spaces and
// newlines.
static string base64DecodeToString(const unsigned char *ubuf, unsigned buflen, string &errorMessage)
{
	...
	for (unsigned i = 0; i < buflen;) {
		...
		while (j < 4 && i < buflen) {
			...
			if (ch == '=') {
				// The '=' at the end of a base64 encoded string indicates size was not evenly divisible
				// by 3 and pre-terminates. We will scan the rest of the string to detect errors.
				for (; i < buflen; i++) {
					if (!(isspace(ubuf[i]) || ubuf[i] == '=')) {
						errorMessage = "found data after '=' terminator in base64 data";
					}
				}
				break;
			}
		...
		// Spit out 3 chars.
		if (j >= 1)
			result.push_back(0xff & (accum >> 16));
		if (j >= 3)
			result.push_back(0xff & (accum >> 8));
		if (j == 4)
			result.push_back(0xff & accum);
	}
	return result;
}
