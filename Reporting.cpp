...
ReportingTable::ReportingTable(const char *filename)
{
	...
	if (rc) {
		gLogEarly(LOG_EMERG | mFacility, "cannot open reporting database at %s, error message: %s", filename,
			sqlite3_errmsg(mDB));
		sqlite3_close(mDB);
		mDB = NULL;
		return;
	...
	if (!sqlite3_command(mDB, createReportingTable)) {
		gLogEarly(LOG_EMERG | mFacility, "cannot create reporting table in database at %s, error message: %s",
			filename, sqlite3_errmsg(mDB));
	}
	// Set high-concurrency WAL mode.
	if (!sqlite3_command(mDB, enableWAL)) {
		gLogEarly(LOG_EMERG | mFacility, "Cannot enable WAL mode on database at %s, error message: %s",
			filename, sqlite3_errmsg(mDB));
	}
...
bool ReportingTable::create(const char *paramName)
{
	...
	if (!sqlite3_command(mDB, cmd)) {
		gLogEarly(LOG_CRIT | mFacility, "cannot create reporting parameter %s, error message: %s", paramName,
			sqlite3_errmsg(mDB));
		return false;
	}
	return true;
...
{
	char cmd[200];
	sprintf(cmd, "UPDATE REPORTING SET VALUE=MAX(VALUE,%u), UPDATETIME=%ld WHERE NAME=\"%s\"", newVal, time(NULL),
		paramName);
	if (!sqlite3_command(mDB, cmd)) {
		gLogEarly(LOG_CRIT | mFacility, "cannot maximize reporting parameter %s, error message: %s", paramName,
			sqlite3_errmsg(mDB));
		return false;
	}
	return true;
...
{
	char cmd[200];
	sprintf(cmd, "UPDATE REPORTING SET VALUE=0, UPDATETIME=0, CLEAREDTIME=%ld WHERE NAME=\"%s\"", time(NULL),
		paramName);
	if (!sqlite3_command(mDB, cmd)) {
		gLogEarly(LOG_CRIT | mFacility, "cannot clear reporting parameter %s, error message: %s", paramName,
			sqlite3_errmsg(mDB));
		return false;
	}
	return true;
...
bool ReportingTable::commit()
{
	...
	if (oustandingCount > 0) {
		...
		while (mp != oustanding.end()) {
			sprintf(cmd, "UPDATE REPORTING SET VALUE=VALUE+%u, UPDATETIME=%ld WHERE NAME=\"%s\"",
				mp->second, time(NULL), mp->first.c_str());
			if (!sqlite3_command(mDB, cmd)) {
				LOG(CRIT) << "could not increment reporting parameter " << mp->first
