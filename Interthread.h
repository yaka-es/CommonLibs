...
public:
	...
	// These functions are solely for use by InterthreadQueue, necessitated by backward compatibility with
	// PointerFIFO.
...
// If you want to iterate through an InterthreadQueue or InterthreadMap, just get the lock using the appropropriate
// qGetLock method.
...
template <class T, class Fifo = PtrList<T>>
...
public:
	...
	public:
		...
		// This is inside ScopedIterator so that the entire InterthreadQueue is locked while you do whatever it
		// is you are doing. Eg:  { InterthreadQueue<T>::ScopedIterator sit(myinterthreadqueue);  for (unsigned
		// i=0; i<10; i++) { T*foo = sit[i]; ... } }
	...
	// Wait for something on either of the two queues connected by iqConnect.  Kind of hokey, but it works.  Timeout
	// is in msecs.
	...
	*/
	T *read(unsigned timeout)
	{
		...
		while (mQ.size() == 0) {
			...
			// (pat) How high do we expect the precision here to be?  I dont think they used precision
			// timers, so dont try to wait if the remainder is just a few msecs.
			if (remaining < 2) {
				return NULL;
			}
...
public:
	...
	*/
	T *read(unsigned timeout)
	{
		...
		while (mQ.size() == 0) {
			long remaining = waitTime.remaining();
			// (pat) How high do we expect the precision here to be?  I dont think they are used as
			// precision timers, so dont try to wait if the remainder is just a few msecs.
			if (remaining < 2) {
				return NULL;
			}
...
public:
	...
	*/
	...
	{
		if (timeout == 0)
			return getNoBlock(key, result, bRemove);
		ScopedLock lock(mLock);
		Timeval waitTime(timeout);
		while (1) {
			typename Map::iterator iter = mMap.find(key);
	...
	*/
	...
	// Caller must use ScopedIterator or the modification lock or enclose the entire iteration in some higher level
	// lock.
...
template <class K, class D>
class InterthreadMap : public InterthreadMap1<K, D *> {
	void vdelete(D *foo) { delete foo; }
};

/** This class is used to provide pointer-based comparison in priority_queues. */
// The priority_queue sorts the largest element to the 'top' of the priority_queue, which is the back of the underlying
// vector.
template <class T>
