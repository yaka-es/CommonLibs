...
SocketError::SocketError() { LOG(DEBUG) << "SocketError"; }
...
DatagramSocket::DatagramSocket() { memset(mDestination, 0, sizeof(mDestination)); }

void DatagramSocket::nonblocking() { fcntl(mSocketFD, F_SETFL, O_NONBLOCK); }

void DatagramSocket::blocking() { fcntl(mSocketFD, F_SETFL, 0); }

void DatagramSocket::close() { ::close(mSocketFD); }
...
int DatagramSocket::write(const char *message, size_t length)
{
	...
	if (retVal == -1)
		perror("DatagramSocket::write() failed");
	return retVal;
}
...
int DatagramSocket::send(const struct sockaddr *dest, const char *message, size_t length)
{
	...
	if (retVal == -1)
		perror("DatagramSocket::send() failed");
	return retVal;
}
...
int DatagramSocket::read(char *buffer)
{
	...
	int length = recvfrom(mSocketFD, (void *)buffer, MAX_UDP_LENGTH, 0, (struct sockaddr *)&mSource, &temp_len);
...
UDPSocket::UDPSocket(unsigned short wSrcPort) : DatagramSocket() { open(wSrcPort); }

UDPSocket::UDPSocket(unsigned short wSrcPort, const char *wDestIP, unsigned short wDestPort) : DatagramSocket()
{
	open(wSrcPort);
...
UDDSocket::UDDSocket(const char *localPath, const char *remotePath) : DatagramSocket()
{
	if (localPath != NULL)
		open(localPath);
	if (remotePath != NULL)
