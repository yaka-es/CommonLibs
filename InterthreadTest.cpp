...
public:
	TestTime(int wFN = 0, int wTN = 0) : mFN(wFN), mTN(wTN) {}

	bool operator<(const TestTime &other) const
	{
		if (mFN == other.mFN)
			return (mTN < other.mTN);
	...
	bool operator==(const TestTime &other) const { return (mFN == other.mFN) && (mTN == other.mTN); }
};
...
struct CompareAdapter {
	...
	bool operator()(const TestTime *rach1, const TestTime *rach2) { return *rach1 > *rach2; }
};
