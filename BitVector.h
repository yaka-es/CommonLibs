...
public:
	...
	// Previous code was often ambiguous, especially for L3Frame and descendent constructors, leading to latent
	// bugs.
	...
	const BitVector cloneSegment(size_t start, size_t span) const
	{
		...
		// (pat) We are depending on the Return Value Optimization not to invoke the copy-constructor on the
		// result, which would result in its immediate destruction while we are still using it.
		BitVector result;
		result.clone(seg);
		return result;
	}
