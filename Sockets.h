...
public:
	...
	// class ConfigurationTable (needed by the call to gConfig) and the class containing the UDPSocket and calling
	// gConfig. Alternatively, since we cannot add an empty constructor (because it is ambiguous with the following)
	...
	UDPSocket(unsigned short localPort, const char *remoteIP, unsigned short remotePort);
