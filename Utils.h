...
inline string format(const char *fmt, string s1) { return format1(fmt, s1.c_str()); }
inline string format(const char *fmt, string s1, string s2) { return format1(fmt, s1.c_str(), s2.c_str()); }
...
inline string format(const char *fmt, string s1, int i1) { return format1(fmt, s1.c_str(), i1); }
inline string format(const char *fmt, int i1, string s1) { return format1(fmt, i1, s1.c_str()); }
inline string format(const char *fmt, string s1, string s2, int i1) { return format1(fmt, s1.c_str(), s2.c_str(), i1); }
...
struct Statistic {
	...
	//	return (std::string)mCurrent + " min=" + (std::string) mMin +" max="+(string)mMax+ format(" avg=%4g
	// sd=%3g",getAvg(),getSD());
	//}
};
...
class RefCntBase {
	...
	// Actually, you could copy it, if the refcnt in the new object were 0 and the pointer was assigned to a
	// RefCntPointer.
	RefCntBase(RefCntBase &) { assert(0); }
	RefCntBase(const RefCntBase &) { assert(0); }
	RefCntBase operator=(RefCntBase &)
	{
		assert(0);
		mRefCnt = 0;
		return *this;
	}
	// These are not 'const' but wonderful C++ requires us to declare them const so we can use a const pointer as
	// the RefCntPointer target, then add 'mutable' to everything.  What a botched up language.
...
public:
	...
	RefCntPointer(const RefCntPointer &other)
	{
		rcPointer = other.rcPointer;
		rcInc();
	}
	RefCntPointer(RefCntPointer &other)
	{
		rcPointer = other.rcPointer;
		rcInc();
	}
	RefCntPointer(Type *other)
	{
		rcPointer = other;
		rcInc();
	}
	// (pat) Making this virtual ~RefCntPointer makes ~MMContext crash, dont know why.
	// (pat) Update: lets try making it virtual again; it should work, but no one derives off RefCntPointer so I
	// dont know why it crashes..
	~RefCntPointer()
	{
		if (rcPointer) {
			LOG(DEBUG) << " rcPointer=" << ((void *)rcPointer)
				   << " refcnt=" << (rcPointer ? rcPointer->getRefCnt() : 0);
		}
		rcDec();
	...
	RefCntPointer<Type> &operator=(RefCntPointer other)
	{
		other.rcInc();
		rcDec();
		rcPointer = other.rcPointer;
	...
	// The old is the previous value of this pointer.  We do not decrement the refcnt in the other that we are
	// taking charge of.
	RefCntPointer<Type> &operator=(Type *other)
	{
		Type *old = rcPointer;
		rcPointer = other;
		rcInc();
	...
	Type &operator*() const { return *rcPointer; }
	// This auto-conversion causes gcc warning messages, which are of no import, but to avoid them I now use self()
	// everywhere instead.
	// operator Type*() const { return rcPointer; }
	// free is a synonym for *this = NULL; but it is a better comment in the code what is happening.  RefCntBase is
	// only freed if this was the last pointer to it.
	void free()
	{
		int refcnt = rcPointer ? rcPointer->getRefCnt() : 0;
		rcDec();
		if (refcnt > 1) {
			LOG(DEBUG) << "RefCntPointer " << (void *)rcPointer << " refcnt before=" << refcnt
				   << " did not free structure.";
		} else {
			LOG(DEBUG) << "RefCntPointer " << (void *)rcPointer << "  refcnt before=" << refcnt;
		}
		rcPointer = NULL;
