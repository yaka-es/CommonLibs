...
extern pid_t gPid;
#define _LOG(level) \
	Log(LOG_##level).get() << gPid << ":" << gettid() << Utils::timestr(100, true) << " " __FILE__ ":" << __LINE__ \
			       << ":" << __FUNCTION__ << ": "
...
#define OBJLOG(wLevel) LOG(wLevel) << "obj: " << this << ' '

#define LOG_ASSERT(x) \
	{ \
		if (!(x)) \
			LOG(EMERG) << "assertion " #x " failed"; \
	} \
	assert(x);
...
public:
	Log(int wPriority) : mPriority(wPriority), mDummyInit(false) {}
