	...
	for (unsigned len = 0; len < 100; len++) { // Length of binary data.
		for (unsigned testno = 0; testno < 100; testno++) {
			...
			for (unsigned type = 0; type < 2; type++) {
				...
				if (testno & 1)
					encoded = messup(encoded);
				string decoded = decodeToString(encoded.data(), encoded.size(), method, errorMessage);

				// Check our results.
				if (decoded.size() != len) {
					printf("FAIL %s len=%d size=%zu encoded=%s\n", method, len, decoded.size(),
						encoded.c_str());
				}
				if (binaryinput != decoded) {
					printf("FAIL %s testno=%d len=%d encoded=%s\n", method, testno, len,
						encoded.c_str());
				}
				// Paranoid double check manually:
				for (unsigned j = 0; j < len; j++) {
					if (tbuf[j] != decoded[j]) {
						printf("FAIL %s len=%d j=%d\n", method, len, j);
					}
				}
				...
				if (verbose)
					printf("TEST %s len=%d test=%d encoded=%s\n", method, len, testno,
						encoded.c_str());
			}
			...
			{	// Hex encoder/decoder tests.
				...
				string hexdecoded =
			decodeToString(hexencoded.data(),hexencoded.size(),"hex",errorMessage);
				//printf("input size=%d hexencoded size=%d hexdecoded size=%d
			%s\n",len,hexencoded.size(),hexdecoded.size(),hexencoded.c_str()); string e2 =
			encodeToString(hexdecoded.data(),hexdecoded.size(),"hex",errorMessage);
