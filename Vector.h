...
// transparently in sometimes surprising places, this made the
//		class very difficult to use.  Moreover, since the C++ standard specifies that a copy-constructor is used
//		to copy the return value from functions, it makes it literally impossible for a function to fully
// control
//		the return value.  Our code has relied on the "Return Value Optimization" which says that the C++
// compiler
//		may omit the copy-construction of the return value even if the copy-constructor has side-effects, which
// ours does. 		This methodology is fundamentally incompatible with C++.
...
// non-const
// case. 			This is really horrible. 		The segment methods were identical for const and
// non-const cases, always returning an alias. 		This also resulted in zillions of redundant mallocs and copies
// throughout the code base. 	class BitVector: 		Copy-constructor:
...
// conversion
//			of the result of the const segment() method from Vector to BitVector which caused the Vector
// copy-constructor
//			to be (inadvertently) invoked, resulting in the const version of the segment method returning a
// clone.
...
// classes,
//		or any method that returns a VectorBase type object.  Why?  Because any of the above when used in
// derived  classes 		can cause copy-constructor invocation, often surprisingly, obfuscating the code.
//		Each derived class must provide its own: copy-constructors and segment() and related methods, since we
// do  not 		want to inadvertently invoke a copy-constructor to convert the segment() result from VectorBase
// to the  derived type. 	BitVector: 		The BitVector copy-constructor and assignment operator
// (inherited from VectorBase) paradigm is: 		if the copied Vector owned memory, perform a clone so the new
// vector owns memory
// also, 		otherwise just do a simple copy, which is another alias.  This isnt perfect but works every
// place 		in our code base and easier to use than the previous paradigm.
...
// well, we should use refcnts. 	Vector: 		I left Vector alone (except for rearrangement to
// separate out  VectorBase.)  Vector should just not be used. 	SoftVector: 		SoftVector and signalVector
// should be updated similar to BitVector, but I did not want to disturb them. What the behavior should be:
